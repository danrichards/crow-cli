...
class ReadCommandTest extends TestCase
{
    ...
    public function test_read_by_id_ignores_unread_lookup_and_leave_unread_skips_mark_read(): void
    {
        ...
        $this->artisan('read evt_123 --leave-unread --json')
    ...
    public function test_read_compatibility_alias_reads_event(): void
    {
        config()->set('crow.api_token', 'token');

        Http::fake([
            'crow.test/api/v1/listener-events/evt_alias' => Http::response([
                'data' => [
                    'id' => 'evt_alias',
                    'type' => 'dispatch.received',
                    'payload' => [
                        'kind' => 'dispatch',
                        'dispatch' => ['title' => 'Alias event'],
                    ],
                ],
            ]),
            'crow.test/api/v1/listener-events/evt_alias/read' => Http::response(['data' => []]),
        ]);

        $this->artisan('crow:read evt_alias')
            ->expectsOutputToContain('Crow Dispatch: Alias event')
            ->assertExitCode(0);
    }
}
