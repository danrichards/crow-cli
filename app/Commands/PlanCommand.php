...
class PlanCommand extends Command
{
    ...
    private function handlePlanList(CrowApiClient $client, PlanHandoffFormatter $formatter): int
    {
        ...
        if (is_string($this->option('output')) && trim((string) $this->option('output')) !== '') {
            return $this->writeOutput($formatter->planListMarkdown($payload, $this->stringOption('api-url')), 'Crow plan list');
        }
        ...
        $this->line('Run `crow plan <plan-id>` to fetch a handoff.');
        $this->line('View plans on the web: '.$this->plansWebUrl(app(CrowConfig::class)));
    ...
    private function plansWebUrl(CrowConfig $config): string
    {
        $base = rtrim($config->apiUrl($this->stringOption('api-url')), '/');
