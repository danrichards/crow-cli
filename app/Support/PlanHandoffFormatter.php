...
{
    public function __construct(private ?CrowConfig $config = null) {}
    ...
    public function planListMarkdown(array $payload, ?string $apiUrl = null): string
    {
        ...
        $lines[] = '';
        $lines[] = 'Run `crow plan <plan-id>` to fetch a handoff.';
        $lines[] = 'View plans on the web: '.$this->plansWebUrl($apiUrl);
    ...
    private function plansWebUrl(?string $apiUrl = null): string
    {
        $base = rtrim($this->config?->apiUrl($apiUrl) ?? (string) config('crow.default_api_url'), '/');
